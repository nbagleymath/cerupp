    ...
    for it = 2:nt
        ...
        if use_plasma_time_rk2
            ...
            [rho_next, drho_ofi_step, drho_aval_step, drho_recomb_step, next_clamp_mask, ...
                drho_ofi_rejected_step, drho_aval_rejected_step, ...
                drho_recomb_limited_step, drho_numerical_residual_step, ...
                diag_local, plasma_softwarn_state] = ...
                advance_rho_stage_local( ...
        ...
        if need_stage_mechanism_ledgers
            ...
            % drho_ofi_accepted is the gross OFI event-density increment
            % applied by the plasma update after the source-reservoir rule,
            % rho_stage_cap, exact-stage replay, and numerical clipping. The
            % OFI event-depletion NLA path consumes this event record only;
            % avalanche and recombination stay in their own density records
            % and are not multiplied by Udep_matched for OFI loss.
            valid_ledger_core = isfinite(drho_ofi_step);
            plasma_book.drho_ofi_accepted(:, it) = drho_ofi_step;
            plasma_book.drho_ofi_applied(:, it) = drho_ofi_step;
            plasma_book.drho_aval_accepted(:, it) = drho_aval_step;
            plasma_book.drho_aval_applied(:, it) = drho_aval_step;
            plasma_book.drho_ofi_rejected_by_cap(:, it) = drho_ofi_rejected_step;
            plasma_book.drho_aval_rejected_by_cap(:, it) = drho_aval_rejected_step;
            plasma_book.drho_recomb_limited_by_floor(:, it) = drho_recomb_limited_step;
            plasma_book.drho_numerical_residual(:, it) = drho_numerical_residual_step;
            if isfield(plasma_book, 'drho_recomb_applied')
                plasma_book.drho_recomb_accepted(:, it) = drho_recomb_step;
                plasma_book.drho_recomb_applied(:, it) = drho_recomb_step;
            end
            plasma_book.exact_valid_core(:, it) = valid_ledger_core;
            plasma_book.ledger_cap_core(:, it) = logical(next_clamp_mask);
            plasma_book.ledger_clip_core(:, it) = logical(next_clamp_mask);
            plasma_book.ledger_repair_core(:, it) = ~valid_ledger_core;
        end
    end
...
function [rho_out, drho_ofi_step, drho_aval_step, drho_recomb_step, clamp_mask, ...
    drho_ofi_rejected_step, drho_aval_rejected_step, drho_recomb_limited_step, ...
    drho_numerical_residual_step, diag_local, plasma_softwarn_state] = ...
    advance_rho_stage_local( ...
...
%   documented in advance_z_step.m.
    ...
    drho_ofi_rejected_step = zeros(size(rho_prev), 'like', rho_prev);
    drho_aval_rejected_step = zeros(size(rho_prev), 'like', rho_prev);
    drho_recomb_limited_step = zeros(size(rho_prev), 'like', rho_prev);
    drho_numerical_residual_step = zeros(size(rho_prev), 'like', rho_prev);
    % Stage dispatcher:
    ...
    if any(clamp_mask(:))
        ...
        if strcmp(stage_tag, 'full') && need_stage_mechanism_ledgers
            [drho_ofi_step, drho_aval_step, drho_recomb_step, ...
                drho_ofi_rejected_step, drho_aval_rejected_step, ...
                drho_recomb_limited_step, drho_numerical_residual_step] = ...
                accepted_mechanism_ledgers_after_stage_clip_local( ...
                    rho_prev, rho_out, rho_unclamped, drho_ofi_step, ...
                    drho_aval_step, drho_recomb_step, rho_stage_cap);
        end
    end
end

function [drho_ofi_acc, drho_aval_acc, drho_recomb_acc, ...
    drho_ofi_rejected, drho_aval_rejected, drho_recomb_limited, ...
    drho_numerical_residual] = ...
    accepted_mechanism_ledgers_after_stage_clip_local( ...
    rho_prev, rho_clipped, rho_unclipped, drho_ofi_raw, ...
    drho_aval_raw, drho_recomb_raw, rho_stage_cap)
% Attribute stage clipping to accepted mechanism ledgers.
% Upper caps scale positive OFI/avalanche source increments in proportion to
% their raw proposal. A lower floor limits recombination to the amount that
% can be removed without driving rho negative. The result reproduces the
% clipped discrete rho update; it is not a continuum material statement.

    drho_ofi_acc = drho_ofi_raw;
    drho_aval_acc = expand_stage_ledger_like_local(drho_aval_raw, rho_prev);
    drho_recomb_acc = expand_stage_ledger_like_local(drho_recomb_raw, rho_prev);
    drho_ofi_rejected = zeros(size(rho_prev), 'like', rho_prev);
    drho_aval_rejected = zeros(size(rho_prev), 'like', rho_prev);
    drho_recomb_limited = zeros(size(rho_prev), 'like', rho_prev);
    drho_numerical_residual = zeros(size(rho_prev), 'like', rho_prev);
    clip_mask = (rho_unclipped < 0) | (rho_unclipped > rho_stage_cap);
    if ~any(clip_mask(:))
        return;
    end
    if any(~isfinite(drho_ofi_acc(clip_mask))) || ...
            any(~isfinite(drho_aval_acc(clip_mask))) || ...
            any(~isfinite(drho_recomb_acc(clip_mask)))
        nan_like = cast(NaN, 'like', rho_prev);
        drho_ofi_acc(clip_mask) = nan_like;
        drho_aval_acc(clip_mask) = nan_like;
        drho_recomb_acc(clip_mask) = nan_like;
        drho_numerical_residual(clip_mask) = nan_like;
        return;
    end

    zero_like = cast(0, 'like', rho_prev);
    eps_like = cast(eps('double'), 'like', rho_prev);
    drho_ofi_raw_full = drho_ofi_acc;
    drho_aval_raw_full = drho_aval_acc;
    drho_recomb_raw_full = drho_recomb_acc;
    upper_mask = clip_mask & (rho_unclipped > rho_stage_cap);
    if any(upper_mask(:))
        proposed_source = max(drho_ofi_acc, zero_like) + ...
            max(drho_aval_acc, zero_like);
        accepted_source = max(rho_stage_cap - rho_prev + ...
            max(drho_recomb_acc, zero_like), zero_like);
        source_scale = min(cast(1, 'like', rho_prev), ...
            accepted_source ./ max(proposed_source, eps_like));
        drho_ofi_acc(upper_mask) = source_scale(upper_mask) .* ...
            max(drho_ofi_acc(upper_mask), zero_like);
        drho_aval_acc(upper_mask) = source_scale(upper_mask) .* ...
            max(drho_aval_acc(upper_mask), zero_like);
        drho_ofi_rejected(upper_mask) = max( ...
            drho_ofi_raw_full(upper_mask) - drho_ofi_acc(upper_mask), ...
            zero_like);
        drho_aval_rejected(upper_mask) = max( ...
            drho_aval_raw_full(upper_mask) - drho_aval_acc(upper_mask), ...
            zero_like);
    end

    lower_mask = clip_mask & (rho_unclipped < 0);
    if any(lower_mask(:))
        proposed_source = max(drho_ofi_acc, zero_like) + ...
            max(drho_aval_acc, zero_like);
        allowed_recomb = max(rho_prev + proposed_source, zero_like);
        drho_recomb_acc(lower_mask) = min( ...
            max(drho_recomb_acc(lower_mask), zero_like), ...
            allowed_recomb(lower_mask));
        drho_recomb_limited(lower_mask) = max( ...
            drho_recomb_raw_full(lower_mask) - drho_recomb_acc(lower_mask), ...
            zero_like);
    end

    residual = rho_clipped - (rho_prev + drho_ofi_acc + ...
        drho_aval_acc - drho_recomb_acc);
    drho_numerical_residual(clip_mask) = residual(clip_mask);
    residual_tol = cast(128 .* eps('double'), 'like', rho_prev) .* ...
        max(cast(1, 'like', rho_prev), max(abs(rho_clipped), abs(rho_prev)));
    bad_residual = clip_mask & (abs(residual) > residual_tol);
    if any(bad_residual(:))
        nan_like = cast(NaN, 'like', rho_prev);
        drho_ofi_acc(bad_residual) = nan_like;
        drho_aval_acc(bad_residual) = nan_like;
        drho_recomb_acc(bad_residual) = nan_like;
        drho_numerical_residual(bad_residual) = nan_like;
    end
end

function ledger = expand_stage_ledger_like_local(ledger_in, like_template)
% Broadcast scalar mechanism ledgers to the active density shape.

    if isscalar(ledger_in)
        ledger = ledger_in + zeros(size(like_template), 'like', like_template);
    else
        ledger = ledger_in;
    end
end
...
% step's start/predictor stages when that heavier ledger is enabled.
    ...
    if ~plasma_book_workspace_matches_local(plasma_book, like_template, nc, nt, full_bookkeeping_ledger)
        ...
        plasma_book.drho_ofi_accepted(:) = 0;
        plasma_book.drho_ofi_applied(:) = 0;
        plasma_book.drho_aval_accepted(:) = 0;
        plasma_book.drho_aval_applied(:) = 0;
        plasma_book.drho_ofi_rejected_by_cap(:) = 0;
        plasma_book.drho_aval_rejected_by_cap(:) = 0;
        plasma_book.drho_recomb_limited_by_floor(:) = 0;
        plasma_book.drho_numerical_residual(:) = 0;
        plasma_book.exact_valid_core(:) = true;
        plasma_book.ledger_cap_core(:) = false;
        plasma_book.ledger_clip_core(:) = false;
        plasma_book.ledger_repair_core(:) = false;
        if full_bookkeeping_ledger
            plasma_book.drho_recomb_accepted(:) = 0;
...
% Check whether the carried plasma-book arrays can be reused on this stage.
    ...
    if ~isstruct(plasma_book) || ...
            ~isfield(plasma_book, 'drho_ofi_accepted') || ...
            ~isfield(plasma_book, 'drho_aval_accepted') || ...
            ~isfield(plasma_book, 'drho_ofi_applied') || ...
            ~isfield(plasma_book, 'drho_aval_applied') || ...
            ~isfield(plasma_book, 'exact_valid_core') || ...
            ~isfield(plasma_book, 'ledger_cap_core') || ...
            ~isfield(plasma_book, 'ledger_clip_core') || ...
            ~isfield(plasma_book, 'ledger_repair_core') || ...
            ~isfield(plasma_book, 'drho_ofi_rejected_by_cap') || ...
            ~isfield(plasma_book, 'drho_aval_rejected_by_cap') || ...
            ~isfield(plasma_book, 'drho_recomb_limited_by_floor') || ...
            ~isfield(plasma_book, 'drho_numerical_residual')
        return;
    end
    if ~isequal(size(plasma_book.drho_ofi_accepted), [nc, nt]) || ...
            ~isequal(size(plasma_book.drho_aval_accepted), [nc, nt]) || ...
            ~isequal(size(plasma_book.drho_ofi_applied), [nc, nt]) || ...
            ~isequal(size(plasma_book.drho_aval_applied), [nc, nt]) || ...
            ~isequal(size(plasma_book.exact_valid_core), [nc, nt]) || ...
            ~isequal(size(plasma_book.ledger_cap_core), [nc, nt]) || ...
            ~isequal(size(plasma_book.ledger_clip_core), [nc, nt]) || ...
            ~isequal(size(plasma_book.ledger_repair_core), [nc, nt]) || ...
            ~isequal(size(plasma_book.drho_ofi_rejected_by_cap), [nc, nt]) || ...
            ~isequal(size(plasma_book.drho_aval_rejected_by_cap), [nc, nt]) || ...
            ~isequal(size(plasma_book.drho_recomb_limited_by_floor), [nc, nt]) || ...
            ~isequal(size(plasma_book.drho_numerical_residual), [nc, nt]) || ...
            ~strcmp(class(plasma_book.drho_ofi_accepted), class(like_template)) || ...
            ~strcmp(class(plasma_book.drho_aval_accepted), class(like_template)) || ...
            ~strcmp(class(plasma_book.drho_ofi_rejected_by_cap), class(like_template)) || ...
            ~strcmp(class(plasma_book.drho_aval_rejected_by_cap), class(like_template)) || ...
            ~strcmp(class(plasma_book.drho_recomb_limited_by_floor), class(like_template)) || ...
            ~strcmp(class(plasma_book.drho_numerical_residual), class(like_template)) || ...
    ...
    if full_bookkeeping_ledger
        if ~isfield(plasma_book, 'drho_recomb_accepted') || ...
                ~isfield(plasma_book, 'drho_recomb_applied') || ...
                ~isequal(size(plasma_book.drho_recomb_accepted), [nc, nt]) || ...
                ~isequal(size(plasma_book.drho_recomb_applied), [nc, nt]) || ...
                ~strcmp(class(plasma_book.drho_recomb_accepted), class(like_template)) || ...
                ~strcmp(class(plasma_book.drho_recomb_applied), class(like_template))
            return;
        end
    elseif isfield(plasma_book, 'drho_recomb_accepted') || ...
            isfield(plasma_book, 'drho_recomb_applied')
        return;
    end
    tf = true;
