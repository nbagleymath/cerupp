...
[s_tot, xc, yc, xpk, ypk, rmean_fluence] = deal( ...
    fluence_summary.total, fluence_summary.centroid_x, fluence_summary.centroid_y, ...
    fluence_summary.peak_x, fluence_summary.peak_y, fluence_summary.rmean_fluence);
if rebuild_core
...
% Reload one selected checkpoint payload on demand during rebuild.
    ...
    [rs, ~] = require_restart_payload_groups_local( ...
        s.restart_state, checkpoint_dir, checkpoint_name, struct());
    if ~isfield(rs, 'grid') || ~isstruct(rs.grid) || ...
            ~isfield(rs.grid, 'x') || isempty(rs.grid.x) || ...
            ~isfield(rs.grid, 'y') || isempty(rs.grid.y) || ...
